...
class BinaryClassifiers:
    """Binary Classifiers using CNN Arquitectures"""
    ...
    def get_performance_metrics(self):
        ...
        for model_name, model in zip(
            ...
            y_train_predicted = np.round_(model.predict(self.x_train))
            y_test_predicted = np.round_(model.predict(self.x_test))
            train_metrics = [
                "training",
                accuracy_score(y_true = self.y_train, y_pred = y_train_predicted),
                recall_score(y_true = self.y_train, y_pred = y_train_predicted),
                precision_score(y_true = self.y_train, y_pred = y_train_predicted),
                f1_score(y_true = self.y_train, y_pred = y_train_predicted),
                matthews_corrcoef(y_true = self.y_train, y_pred = y_train_predicted)]
            test_metrics = [
                "testing",
                accuracy_score(y_true = self.y_test, y_pred = y_test_predicted),
                recall_score(y_true = self.y_test, y_pred = y_test_predicted),
                precision_score(y_true = self.y_test, y_pred = y_test_predicted),
                f1_score(y_true = self.y_test, y_pred = y_test_predicted),
